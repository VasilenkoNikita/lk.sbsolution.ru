...
class PaymentsListLayout extends Table
{
    ...
    {
        return [
            TD::make('payment_name', 'Наименование оплаты')
				->sort()
				->filter(TD::FILTER_TEXT)
                ->render(function (Payment $payment) {
                    return Link::make($payment->payment_name)
                        ->route('platform.payments.edit', $payment);
                }),


            TD::make('paymentsTypes.name', 'Тип оплаты')
                ->render(function (Payment $payment) {
                        if(!$payment->paymentsTypes->isEmpty()) {
                            return $payment->paymentsTypes[0]->name;
                        }
                        return "Не указано";
                    }),

            TD::make('paymentsSubtypes.name', 'Вид оплаты')
                ->render(function (Payment $payment) {
                    if(!$payment->paymentsSubtypes->isEmpty()) {
                        return $payment->paymentsSubtypes[0]->name;
                    }
                    return "Не указано";
                }),
            ...
            TD::make('type_of_ownership', 'Форма собственности')
                ->render(function (Payment $payment) {
                    if(!is_null($payment->type_of_ownership)) {
                        return $payment->type_of_ownership;
                    }
                    return "Не указано";
                }),

            TD::make('payment_date', 'Время сдачи отчета')
                ->render(function (Payment $payment) {
